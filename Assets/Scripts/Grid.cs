...
public class Grid
{
    ...
    public Grid(int rows, int column, float cellSize)
    {
        ...
        //Method responsible for carving out the maze
        GenerateMaze(0, 0);
    }
    ...
    public QuadrantBounds GetQuadrantBounds(Quadrant quadrant)
    {
        int midRow = rows / 2;
        int midCol = column / 2;

        QuadrantBounds bounds = new QuadrantBounds();

        switch (quadrant)
        {
            case Quadrant.TopLeft:
                bounds.minX = 0;
                bounds.maxX = midRow - 1;
                bounds.minY = midCol;  
                bounds.maxY = column - 1;
                break;
